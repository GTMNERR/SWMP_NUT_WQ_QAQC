# load libraries and data files data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABIAAAASCAYAAABWzo5XAAAAbElEQVR4Xs2RQQrAMAgEfZgf7W9LAguybljJpR3wEse5JOL3ZObDb4x1loDhHbBOFU6i2Ddnw2KNiXcdAXygJlwE8OFVBHDgKrLgSInN4WMe9iXiqIVsTMjH7z/GhNTEibOxQswcYIWYOR/zAjBJfiXh3jZ6AAAAAElFTkSuQmCC
...
PI3<-PI %>%
  ...
  arrange(Date) %>%
  filter(between(Date, as.Date('2026-01-01'), as.Date('2026-03-31'))) #change these dates each quarter
...
PI3<-PI %>%
  dplyr::mutate(Date = as.Date(Date, format = "%m/%d/%Y")) %>%
        arrange(Date) %>%
        filter(between(Date, as.Date('2025-04-01'), as.Date('2025-06-30'))) #change these dates each quarter
...
PI3<-PI %>%
  dplyr::mutate(Date = as.Date(Date, format = "%m/%d/%Y")) %>%
  arrange(Date) %>%
  filter(between(Date, as.Date('2025-10-01'), as.Date('2025-12-31'))) #change these dates each quarter
...
PI_cdmoFormat <- PI3 %>% 
  dplyr::select(Date, Time, F_Record, Temp, F_Temp,	SpCond,	F_SpCond,	Sal,	F_Sal,	DO_pct,	F_DO_pct,	DO_mgl,	F_DO_mgl,	Depth,	F_Depth,	pH,	F_pH,	Turb,	F_Turb,	ChlFluor,	F_ChlFluor,	fDOMQSU,	fDOMRFU
  ) 
PI_noFlags <- PI3 %>% 
  dplyr::select(Date, Time, Temp, SpCond,	Sal, DO_pct, DO_mgl, Depth,	pH,	Turb,	ChlFluor, fDOMQSU, fDOMRFU
  ) 
...
PC_cdmoFormat <- PC3 %>% 
  dplyr::select(Date, Time, F_Record, Temp, F_Temp,	SpCond,	F_SpCond,	Sal,	F_Sal,	DO_pct,	F_DO_pct,	DO_mgl,	F_DO_mgl,	Level,	F_Level,	pH,	F_pH,	Turb,	F_Turb,	ChlFluor,	F_ChlFluor,	fDOMQSU,	fDOMRFU
  )
PC_noFlags <- PC3 %>% 
  dplyr::select(Date, Time, Temp, SpCond,	Sal, DO_pct, DO_mgl, Level,	pH,	Turb,	ChlFluor, fDOMQSU, fDOMRFU
  )
...
write.csv(PI_cdmoFormat, here::here('output', 'wq', 'data', 'quarterly', 'gtmpiwqQ1_cdmoFormat.csv'), row.names = FALSE)
...
write.csv(FM_noFlags, here::here('output', 'wq', 'data', 'quarterly', 'gtmfmwqQ2_noFlags.csv'), row.names = FALSE)

write.csv(PC3, here::here('output', 'wq', 'data', 'quarterly', 'gtmpcwqQ2_allParameters.csv'), row.names = FALSE)
write.csv(PC_cdmoFormat, here::here('output', 'wq', 'data', 'quarterly','gtmpcwqQ2_cdmoFormat.csv'), row.names = FALSE)
write.csv(PC_noFlags, here::here('output', 'wq', 'data', 'quarterly', 'gtmpcwqQ2_noFlags.csv'), row.names = FALSE)
# 
...
write.csv(PIjan, here::here('output', 'wq', 'data', 'monthly', 'PIJan2026.csv'), row.names = FALSE)
write.csv(PIfeb, here::here('output', 'wq', 'data', 'monthly', 'PIFeb2026.csv'), row.names = FALSE)
write.csv(PImar, here::here('output', 'wq', 'data', 'monthly', 'PIMar2026.csv'), row.names = FALSE)

write.csv(SSjan, here::here('output', 'wq', 'data', 'monthly', 'SSJan2026.csv'), row.names = FALSE)
write.csv(SSfeb, here::here('output', 'wq', 'data', 'monthly', 'SSFeb2026.csv'), row.names = FALSE)
write.csv(SSmar, here::here('output', 'wq', 'data', 'monthly', 'SSMar2026.csv'), row.names = FALSE)

write.csv(FMjan, here::here('output', 'wq', 'data', 'monthly', 'FMJan2026.csv'), row.names = FALSE)
write.csv(FMfeb, here::here('output', 'wq', 'data', 'monthly', 'FMFeb2026.csv'), row.names = FALSE)
write.csv(FMmar, here::here('output', 'wq', 'data', 'monthly', 'FMMar2026.csv'), row.names = FALSE)

write.csv(PCjan, here::here('output', 'wq', 'data', 'monthly', 'PCJan2026.csv'), row.names = FALSE)
write.csv(PCfeb, here::here('output', 'wq', 'data', 'monthly', 'PCFeb2026.csv'), row.names = FALSE)
write.csv(PCmar, here::here('output', 'wq', 'data', 'monthly', 'PCMar2026.csv'), row.names = FALSE)
