...
lims <- readxl::read_xlsx(here::here('data', 
                                     '2025',
                                     'LIMS_Tier3_Only.xlsx'), # this is where you'd want to rename the file
...
lims2 <- lims2 %>%
  dplyr::filter(station_code != "diwater") #& station_code != "disystemcheck1" & station_code != "disystemcheck2" & station_code != "disystemcheck3") #this may have to be changed to fieldblank (no spaces)
