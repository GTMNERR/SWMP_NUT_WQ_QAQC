...
lims_wide_results <- lims3 %>%
  dplyr::select(station_code, datetimestamp, cdmo_name, component_long, test_comments, result) %>%
...
lims_wide <- lims_wide_results %>%
  ...
  tidyr::separate(num,
                  ...
                  sep = "[.]") %>%
  dplyr::select(1, 85, 4, 2:3, 5:84) # may need to modify last number
...
#For Tier 3 Parameters
lims_wide2 <- lims_wide %>%
  dplyr::select(1:5, 85, # reorder everything, 1:5 are the station code, etc. columns
                `2,4-d`, `F_2,4-d`,
                `2,4-db`, `F_2,4-db`,
                ACETA, F_ACETA,
                SUCRA, F_SUCRA,
                ...
                silvex, F_silvex,
                `Acesulfame K`, `F_Acesulfame K`,
                Endothall, F_Endothall,
                AMPA, F_AMPA,
                Glufosinate, F_Glufosinate,
                Glyphosate, F_Glyphosate,
                afidopyropen, F_afidopyropen,
                benzovindiflupyr, F_benzovindiflupyr,
                clothianidin, F_clothianidin,
                dinotefuran, F_dinotefuran,
                mandestrobin, F_mandestrobin,
                thiamethoxam, F_thiamethoxam,
                tolfenpyrad, F_tolfenpyrad,
                acetamiprid, F_acetamiprid,
                `2,4,5-t`, `F_2,4,5-t`,
                picloram, F_picloram,
                mcpa, F_mcpa,
                acifluorfen, F_acifluorfen,
                dicamba, F_dicamba,
                dichlorprop, F_dichlorprop,
                dinoseb, F_dinoseb
  )
...
write.csv(lims_wide_final, here::here('output', 'nut', 'data', 'lims_wide_tier3_100625.csv'), row.names = FALSE)
...
xlsx::write.xlsx(lims_wide_final, here::here('output', 'lims_wide_tier3.xlsx'),
