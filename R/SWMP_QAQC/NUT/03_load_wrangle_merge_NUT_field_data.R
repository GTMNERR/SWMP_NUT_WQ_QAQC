...
#If exporting just field data you will need to convert all columns to a character
# swmp_field = data.frame(lapply(swmp_field, as.character), stringsAsFactors=FALSE)

# Output .csv file for SWmP parameters (if needed)
write.csv(swmp_field, here::here('output', 'nut', 'data', 'swmp_field_data_wide.csv'), row.names = FALSE)

# Output .csv file for all parameters (if needed)
write.csv(env_wide, here::here('output', 'nut', 'data', 'all_field_data_wide.csv'), row.names = FALSE)

## clean up environment
rm(CDMO_format, CDMO_format_Final, env, env_wide, env2, lims_wide_final, swmp_field)